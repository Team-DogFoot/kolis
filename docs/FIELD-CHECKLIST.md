...
## B-1. 2026-09-18~19 실제 작업으로 확인된 것 (HAR 없이 화면·UIA 로)
- [x] 납본자료접수 `online/acq/bodepst/depstrecet/onlineDepstRecet.do` — 화면 전체 IE 모드. 버튼: 찾기·접수번호보기·화면초기화·입력·수정·삭제·납본증명서·교열지출력·원문일괄등록(폴더)·복본조사KEY설정·복본조사·중복조사·일괄반입·발행자구분일괄변경·등록대상처리·일괄변경·등록제외처리·원문정보·선정·비선정·선정출력·전체출력·그리드 설정
- [x] 일괄반입 팝업 필드: 수입년도, 업무구분(`62 | 납본(단행)`), 비고, 첨부파일(찾아보기), 작업대상디렉토리정보 `/Upload1/jangseo/ImpCont/amcont/`. 버튼 반입샘플·반입·닫기. 반입 → "일괄반입이 진행됩니다" 확인 → 수 분 → "반입이 완료되었습니다". 접수번호는 팝업 열 때 생성(빈 접수번호가 생길 수 있음 — 직원 확인)
- [x] 원문일괄등록(폴더) 팝업: 원문유형 콤보(열람), 서버디렉토리 `/jangseo11/R301/S2/2026/`, DEXT5 업로더(드래그 영역), 전송하기·일괄정보입력·원문등록. 결과표 폴더명·정보입력결과·원문등록결과
- [x] 수정 팝업(콘텐츠 수정): 파일 목록(원문유형·파일명·크기·등록일·경로), 원문등록·원문삭제·통합뷰어보기·납본뷰어보기, 이전·다음·저장·닫기. 원문등록 → 내부 등록 팝업(파일추가·전송하기·항목제거·확인·닫기, 표의 원문유형 select)
- [x] 전체출력: 알림 막대 저장 → `ExcelDown<숫자>.xls`(SpreadsheetML). 열 No·ㅁ·접수구분·접수자ID·접수번호·콘텐츠유형·장르·ISBN·발간등록번호·본표제·편권차·표제관련정보·저작자·발행자·발행년·가격·원문갯수·업무구분·콘텐츠ID·담당자·파일명
- [ ] 등록대상처리·등록원부작성·디지털콘텐츠관리·전거 찾기·MODS XML 팝업: 아직 안 봄
- [ ] HAR: IE 모드는 F12 불가 → IEChooser. 아직 한 건도 못 받음
